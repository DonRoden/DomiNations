...
//	public int vide=50;
	...
	public void printBoard() {
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[i].length; j++) {
				if(board[j][i].getType().equals("Montagne")) {
					System.out.print("\t"+ "Mont");
				}
				else if(board[j][i].getType().equals("Prairie")) {
					System.out.print("\t"+ "Prairy");
				}
				else {
					System.out.print("\t"+ board[j][i].getType());
				}
				if(board[j][i].getCrown()!=0) {
					System.out.print(board[j][i].getCrown());
				}
			}
			System.out.println();
		}
